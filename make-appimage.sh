...
rm -f shared/bin/datcord
ln -s datcord-bin shared/bin/datcord
...
for _f in shared/lib/datcord/*; do
    [ -e "$_f" ] || continue
    _bname=$(basename "$_f")
    if [ ! -e "shared/bin/$_bname" ] || [ -L "shared/bin/$_bname" ]; then
        ln -sf ../lib/datcord/"$_bname" "shared/bin/$_bname"
    fi
done

# 3) Change AppRun's MAIN_BIN from "datcord" to "launch-app" so the
#    entry point goes through the tray/singleton manager.
sed -i 's/^MAIN_BIN=datcord/MAIN_BIN=launch-app/' AppRun
